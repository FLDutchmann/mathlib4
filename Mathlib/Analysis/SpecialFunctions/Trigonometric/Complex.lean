...
theorem tan_eq_zero_iff {θ : ℂ} : tan θ = 0 ↔ ∃ k : ℤ, k * π / 2 = θ := by
  ...
  simp [field, mul_comm, eq_comm] -- field_simp gets run six times!!
...
theorem cos_eq_cos_iff {x y : ℂ} : cos x = cos y ↔ ∃ k : ℤ, y = 2 * k * π + x ∨ y = 2 * k * π - x :=
  calc
    ...
    _ ↔ (∃ k : ℤ, y = 2 * k * π + x) ∨ ∃ k : ℤ, y = 2 * k * π - x := by
      apply or_congr <;>
        simp [field, sin_eq_zero_iff, (by norm_num : -(2 : ℂ) ≠ 0), eq_sub_iff_add_eq',
...
theorem cos_eq_iff_quadratic {z w : ℂ} :
  ...
  rw [← sub_eq_zero]
  simp [field, cos, exp_neg, exp_ne_zero] -- field_simp gets run six times!!
